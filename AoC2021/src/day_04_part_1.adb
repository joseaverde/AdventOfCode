...
--     1 12 20 15 19        14 21 16 12  6         2  0 12  3  7
--
-- At this point, the third board wins because it has at last one complete row
...
begin
   ...
   Search_Loop : for Number of Order loop
      for Board of Bingo_Boards loop
         for R in Board'Range(1) loop
            for C in Board'Range(2) loop
               if Board(R, C).Number = Number then
                  Board(R, C).Checked := True;
                  if Check_Bingo(Board, R, C) then
                     Ada.Integer_Text_IO.Put(
                        Get_Score(Board, Board(R, C).Number), Width => 1);
