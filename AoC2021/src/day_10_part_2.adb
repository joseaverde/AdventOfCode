...
procedure Day_10_Part_2 is
   function Parse (Line : String) return Long_Integer is
      Stack : Stack_Type;
   ...
   begin
      for Char of Line loop
         case Char is
            when '(' | '[' | '{' | '<' =>
               Stack.Push(Char);
            when ')' => if Stack.Pop /= '(' then return -1; end if;
            when ']' => if Stack.Pop /= '[' then return -1; end if;
            when '}' => if Stack.Pop /= '{' then return -1; end if;
            when '>' => if Stack.Pop /= '<' then return -1; end if;
   ...
   package Sorter is new Vectors.Generic_Sorting("<");
...
begin
   ...
   while not Ada.Text_IO.End_Of_File(File) loop
      Score := Parse(Ada.Text_IO.Get_Line(File));
      if Score >= 0 then
         Scores.Append(Score);
      end if;
   end loop;
   ...
   Ada.Text_IO.Put_Line(Long_Integer'Image(
      Scores(Positive(Scores.Length)/2+1)));
