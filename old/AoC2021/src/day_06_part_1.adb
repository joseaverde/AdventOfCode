...
begin
   ...
   for I in Integer range 1 .. 80 loop
      for J in Positive range 1 .. Positive(Lanternfish.Length) loop
         if Lanternfish(J) = 0 then
            Lanternfish(J) := 6;
   ...
   Ada.Integer_Text_IO.Put(Integer(Lanternfish.Length), Width => 1);
