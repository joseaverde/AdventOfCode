...
procedure AoC2019_Day02 is
   subtype Memory_Range is Natural range 0 .. 256; -- 2**16 - 1;
   type Integer_Array is
      array (Memory_Range)
      of Natural;

   EXPECTED : constant := 19690720;
   Size     : Natural := 0;
   Dummy    : Character;
   Original : Integer_Array := (others => 0);

   function Evaluate (Noun, Verb : Natural) return Natural is
      Memory : Integer_Array;
      I      : Natural := 0;
   begin
      Memory := Original;
      Memory (1) := Noun;
      Memory (2) := Verb;
      while I < Memory'Length loop
         case Memory (I) is
            when 1  => Memory (Memory (I + 3)) :=
               Memory (Memory (I + 1)) + Memory (Memory (I + 2));
            when 2  => Memory (Memory (I + 3)) :=
               Memory (Memory (I + 1)) * Memory (Memory (I + 2));
            when 99 => exit;
            when others => raise Program_Error with "Invalid instruction!";
         end case;
         I := I + 4;
      end loop;
      return Memory (0);
...
exception
   when End_Error =>
      Ada.Text_IO.Put_Line ("AoC 2019 Day 02 Part 1 ->" &
         Natural'Image (Evaluate (12, 2)));

      for Noun in 0 .. 99 loop
         for Verb in 0 .. 99 loop
            -- Ada.Text_IO.Put_Line (Noun'Image & Verb'Image & Evaluate (Noun,
            -- Verb)'Image);
            if Evaluate (Noun, Verb) = EXPECTED then
               Put_Line ("AoC 2019 Day 02 Part 2 ->" &
                  Natural'Image (100 * Noun + Verb));
               exit;
            end if;
         end loop;
      end loop;
