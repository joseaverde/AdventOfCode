\ --- Day 1: Report Repair ---
\ 
\ After saving Christmas five years in a row, you've decided to
\ take a vacation at a nice resort on a tropical island.
\ Surely, Christmas will go on without you.
\ 
\ The tropical island has its own currency and is entirely
\ cash-only. The gold coins used there have a little picture of
\ a starfish; the locals just call them stars. None of the
...
20 CONSTANT STRING-BUFFER-SIZE
CREATE STRING-BUFFER STRING-BUFFER-SIZE ALLOT
VARIABLE INTEGER-BUFFER 1024 CELLS ALLOT

: TEXT ( del -- ) PAD 258 BL FILL WORD COUNT PAD SWAP MOVE ;

: read-input ( -- )
   \ BEGIN STRING-BUFFER STRING-BUFFER-SIZE ACCEPT CATCH DUP .s cr 0 <> WHILE
   BEGIN BL TEXT PAD .s cr DUP 0 <> WHILE
      0. ROT STRING-BUFFER SWAP >NUMBER 2DROP +
      INTEGER-BUFFER @ 1+ CELLS INTEGER-BUFFER + !
   REPEAT ;

CR read-input ." Ok" CR BYE
