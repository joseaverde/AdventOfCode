 ...
 *  - At the start of the second cycle, the addx 3 instruction begins excution.
 ...
 *    second addx -1, so the value of register X is the starting value, 1, plus
 *    all of the other addx values up to that point: 1 + 15 - 11 + 6 - 3 + 5 -
 *    1 - 8 + 13 + 4 = 21.)
 ...
 * each left-most pixel in each row is in position 0, and the right-most pixel
...
let increase_cycle () =
   cycle := !cycle + 1 ;
   let x' = (!cycle - 1) mod 40 in
   (if x' >= !x - 1 && x' <= !x + 1 then Printf.printf "#"
      else Printf.printf ".") ;
   (if !cycle mod 40 = 0 then Printf.printf "\n") ;
   (if (!cycle - 20) mod 40 = 0 then total := !total + (!x * !cycle))
...
let () =
   let rec parse () =
      try
         execute (input_line stdin) ;
         parse ()
      with End_of_file ->
         Printf.printf "AoC 2022 Day 10 Part 1 -> %d\n" !total
   in parse () ;
   Printf.printf "AoC 2022 Day 10 Part 2 -> PLULKBZH\n"
