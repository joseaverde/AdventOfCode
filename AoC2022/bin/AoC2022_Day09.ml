...
let () =
   ...
   let rec visit v x =
      ...
      |   []   ->
         ...
         visited := x :: !visited ;
         false
      | h :: t -> if h = x then true else visit t x
   in let rec move () =
      try
         let (d, a) = match String.split_on_char ' ' (input_line stdin) with
            | d :: [a] -> (d.[0], int_of_string a)
            |    _     -> assert false
         in for _ = 0 to a - 1 do
            begin
               let (hx, hy) = !head in
               match d with
               | 'U' -> head := (hx, hy + 1)
            ...
            begin
               ...
               if (abs dx) > 1 || (abs dy) > 1 then (
                  ...
                  ignore ( visit !visited !tail ) ) ;
            end
            (* Printf.printf "-(%c) (%d, %d) -> (%d, %d) \n"
                  d (fst !tail) (snd !tail) (fst !head) (snd !head); *)
         done ;
         (* Update tail *)
         move ()
      with End_of_file -> ()
   in move () ;
   (* Printf.printf "[" ;
   List.iter (fun x -> Printf.printf "(%d, %d) " (fst x) (snd x)) !visited ;
   Printf.printf "]\n" ; *)
   Printf.printf "%d\n" (List.length !visited)
*)
...
let () =
   let visited  = ref [(0, 0)]
   and visited' = ref [(0, 0)] in
   let visit v x =
      let rec _visit r x =
         match r with
         |   []   -> v := x :: !v
         | h :: t -> (if h <> x then _visit t x)
      in _visit !v x
   in let rec reverse l r =
      match l with
      |   []   -> r
      | h :: t -> reverse t (h :: r)
   in let rec get_actions r =
      try
         let (dir, q) = match String.split_on_char ' ' (input_line stdin) with
         | d :: [q] -> (d.[0], int_of_string q)
         |        _ -> assert false
         in let rec add r n = if n = 0 then r else add (dir :: r) (n - 1) in
         get_actions (add r q)
      with End_of_file -> reverse r []
   in let actions = get_actions [] in
   
   (* Ropes *)
   let rope = Array.make 2 (0, 0) in
   let rope' = Array.make 10 (0, 0) in
   let rec move visited rope actions =
      match actions with
      |   []   -> ()
      | h :: t ->
         rope.(0) <- (  let (x, y) = rope.(0) in
                        match h with
                        | 'U' -> (x, y + 1)
                        | 'D' -> (x, y - 1)
                        | 'L' -> (x - 1, y)
         ...
         for i = 1 to (Array.length rope) - 1 do
            let (tx, ty) = rope.(i)           in
            let (hx, hy) = rope.(i - 1)       in
            let (dx, dy) = (hx - tx, hy - ty) in
            let cap x = if x >= 1 then 1 else if x <= -1 then -1 else 0 in
            (if abs dx > 1 || abs dy > 1 then
               rope.(i) <- (tx + cap dx, ty + cap dy))
         done ;
         visit visited rope.((Array.length rope) - 1) ;
         (* Printf.printf "%c <- (%d, %d) -> (%d, %d)\n"
            h  (fst rope.(0)) (snd rope.(0))
               (fst rope.(1)) (snd rope.(1)) ; *)
         move visited rope t ;
   in
   move visited rope actions ;
   Printf.printf "AoC 2022 Day 09 Part 1 %d\n" (List.length !visited) ;
   move visited' rope' actions ;
   Printf.printf "AoC 2022 Day 09 Part 2 %d\n" (List.length !visited')
