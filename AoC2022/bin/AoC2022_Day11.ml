...
let print_monkey m =
   Printf.printf "Items:" ;
   (List.iter (Printf.printf " %d") !(m.items)) ;
   Printf.printf "\n"

let factor = ref 1

let () = ignore ( print_list )

let read_monkeys () =
   let rec _read_monkeys r =
      try
         let s = ref "" in
         while !s = "" do
            s := input_line stdin
         ...
         let starting = String.trim (input_line stdin) in
         let operation = String.split_on_char ' ' (trim (input_line stdin)) in
         let test = String.split_on_char ' ' (trim (input_line stdin)) in
         let if_true = String.split_on_char ' ' (trim (input_line stdin)) in
         let if_false = String.split_on_char ' ' (trim (input_line stdin)) in
         let op = match drop (List.length operation - 2) operation with
            | _ :: ["old"] -> ('^', 2)
            | s ::   [c]   -> (s.[0], int_of_string c)
            |           _  -> assert false in
         let if_t = match drop (List.length if_true - 1) if_true with
            | [m] -> int_of_string m
            |  _  -> assert false in
         let if_f = match drop (List.length if_false - 1) if_false with
            | [m] -> int_of_string m
            |  _  -> assert false in
         let div = match drop (List.length test - 1) test with
            | [d] -> int_of_string d
            |  _  -> assert false in
         let start = match drop 1 (String.split_on_char ':' starting) with
            | [s] -> String.split_on_char ',' s
            |  _  -> assert false in
         let st = List.init (List.length start) (
            fun i -> let n = List.nth start i in int_of_string (String.trim n))
         in let (m : monkey) = {
            items     = ref st ;
            operation = (match op with
               | ('*', n) -> (fun x -> x * n)
               | ('+', n) -> (fun x -> x + n)
               | ('^', _) -> (fun x -> x * x)
               | ( _,  _) -> assert false) ;
            test      = (fun x -> if x mod div = 0 then if_t else if_f) } in
         if !factor mod div <> 0 then factor := !factor * div ;
         _read_monkeys (m :: r)
      with End_of_file -> r
   in _read_monkeys []

let best2 (a : int array) =
   let b1 = ref 0 in
   let b2 = ref 0 in
   for i = 0 to Array.length a - 1 do
      if a.(i) > !b1 then (b2 := !b1 ; b1 := a.(i))
      else if a.(i) > !b2 then (b2 := a.(i))
         else ()
   done ;
   (!b1, !b2)

let () =
   let m = List.rev (read_monkeys ()) in
   let monkeys = Array.of_list m in
   let length = Array.length monkeys in
   let (monkeys' : monkey array) = Array.init length (fun i ->
      let (m : monkey) = {
         items     = ref !(monkeys.(i).items) ;
         operation = monkeys.(i).operation ;
         test      = monkeys.(i).test} in m) in
   let times = Array.make length 0 in
   for _ = 0 to 19 do
      for i = 0 to length - 1 do
         Printf.printf "Monkey %d . " i ;
         print_monkey monkeys.(i) ;
         List.iter (fun w ->
                    let w' = (monkeys.(i).operation w) / 3 in
                    let j = monkeys.(i).test w' in
                    monkeys.(j).items := w' :: !(monkeys.(j).items))
                    !(monkeys.(i).items) ;
         times.(i) <- times.(i) + List.length !(monkeys.(i).items) ;
         monkeys.(i).items := []
      done ;
      Printf.printf "\n" ;
   done ;
   let (b1, b2) = best2 times in
   Printf.printf "%d\n" (b1 * b2) ;
   let monkeys = Array.copy monkeys' in
   let times = Array.make length 0 in
   for _ = 0 to 9999 do
      for i = 0 to length - 1 do
         Printf.printf "Monkey %d . " i ;
         print_monkey monkeys.(i) ;
         List.iter (fun w ->
                    let w' = (monkeys.(i).operation w) mod !factor in
                    let j = monkeys.(i).test w' in
                    monkeys.(j).items := w' :: !(monkeys.(j).items))
                    !(monkeys.(i).items) ;
         times.(i) <- times.(i) + List.length !(monkeys.(i).items) ;
