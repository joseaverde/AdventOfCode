...
let () =
   ...
   and b2 = ref 0
   and b3 = ref 0 in
   let rec get_calories r =
      try
         let s = input_line stdin in
         if String.length s = 0 then (r, false)
            else get_calories ((int_of_string s) + r)
      with End_of_file -> (r, true)
   in let rec get_all () =
      let (r, s) = get_calories 0 in (
      if      r > !b1 then (b3 := !b2 ; b2 := !b1 ; b1 := r)
      else if r > !b2 then (b3 := !b2 ; b2 := r)
      else if r > !b3 then (b3 := r)) ;
      if s then () else get_all ()
   in get_all () ;
      Printf.printf "AoC 2022 Day 01 Part 1 -> %d\n" !b1 ;
      Printf.printf "AoC 2022 Day 01 Part 2 -> %d\n" (!b1 + !b2 + !b3)
