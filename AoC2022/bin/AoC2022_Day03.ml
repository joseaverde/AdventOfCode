 ...
 * meant to go into exactly one of the two compartments. The Elf that did th
...
let item_value c =
   match c with
   | 'a' .. 'z' -> 1 lsl (Char.code c - Char.code 'a' + 1)
   | 'A' .. 'Z' -> 1 lsl (Char.code c - Char.code 'A' + 27)
   |     _      -> raise Not_found

let encode s =
   let l = String.length s in
   let rec _encode r i =
      if i < l then _encode (r lor item_value s.[i]) (i + 1)
         else r
   in _encode 0 0
...
let log2 n =
   let r = ref 0 in
   for i = 0 to 64 do
      if n lsr i = 0 && !r = 0 then r := i
   done;
   if !r = -1 then 0 else !r - 1

let () =
   let rec _get_lines r =
      try
         let s = input_line stdin in
         let l = String.length s / 2 in
         let a = encode (String.sub s 0 l) in
         let b = encode (String.sub s l (String.length s - l)) in
         _get_lines ((a, b) :: r)
      with End_of_file -> r
   in let rec _part1 r s =
      match r with
      |       []   -> s
      | (a,b) :: t -> _part1 t (s + log2 (a land b))
   in let rec _part2 r s =
      match r with
      |                 []                 -> s
      | (a1,b1) :: (a2,b2) :: (a3,b3) :: t ->
         _part2 t (s + log2 ((a1 lor b1) land (a2 lor b2) land (a3 lor b3)))
      | _ :: _ :: _ -> raise Not_found
      | _ :: _      -> raise Not_found
   in let l = _get_lines [] in
   Printf.printf "AoC 2022 Day 03 Part 1 -> %d\n" (_part1 l 0) ;
   Printf.printf "AoC 2022 Day 03 Part 2 -> %d\n" (_part2 l 0)

(* Original solution
exception Continue

let () =
   let t = ref 0 in     (* Part 1 *)
   let rec _get_line () =
      try
         let s = input_line stdin in
         let l = String.length s / 2 in
         for i = 0 to l - 1 do
            if String.contains_from s l s.[i]
               then begin
                  t := !t + item_value s.[i] ;
                  raise Continue
               end
